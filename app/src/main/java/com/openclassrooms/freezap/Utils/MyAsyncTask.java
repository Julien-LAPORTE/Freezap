...
public class MyAsyncTask extends AsyncTask<Void, Void, Long> {
    ...
    // 2 - Declare callback
    private final WeakReference<Listeners> m_callback;

    // 3 - Constructor
    public MyAsyncTask(Listeners callback){
        m_callback = new WeakReference<>(callback);
    }

    @Override
    protected void onPreExecute() {
        super.onPreExecute();
        m_callback.get().onPreExecute(); // 4 - Call the related callback method
    ...
    protected void onPostExecute(Long success) {
        ...
        m_callback.get().onPostExecute(success); // 4 - Call the related callback method
        Log.e("TAG", "AsyncTask is finished.");
    }

    @Override
    protected Long doInBackground(Void... voids) {
        m_callback.get().doInBackground(); // 4 - Call the related callback method
